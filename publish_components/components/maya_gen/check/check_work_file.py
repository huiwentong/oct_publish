...
def main(submit_data:dict, process_data:dict, parent_widget=None, logger=None):
    ...
    try:
        ...
        if context_info:
            if context_info['disk'] == 'W:':
                if context_info['proj'].lower() == tc.project_name.lower() and \
                        ...
                        context_info['step'] == tc.step_name.lower() and \
        ...
        if not is_right_path:
            ...
            correct_path = os.path.join(work_dir, dcc.lower(), version_name + '.' + ext)
            return u"当前文件的路径和名称不符合规范，正确的文件应该是{}。".format(correct_path)

        work_file_name = os.path.basename(work_file)
        if work_file_name.rsplit('.', 1)[0] != version_name:
            return u"工程文件名 {} 和版本名 {} 不一致".format(work_file_name, version_name)
